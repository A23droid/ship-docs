...
# Fix: Capitalize FastAPI
app = FastAPI()

class ShipDocsRequest(BaseModel):
    github_url: HttpUrl
    description: Optional[str] = Field(None, description="Optional extra business/project context from the user")
...
async def process_url(request: ShipDocsRequest):
    ...
    # Fix: Cast HttpUrl to string before parsing components
    ...
    # 3. Open the non-blocking client connection block
    async with httpx.AsyncClient(timeout=10.0) as client:
        try:
            # STEP A: Fetch repository metadata to find the default branch name dynamically
            meta_url = f"https://api.github.com/repos/{owner}/{repo_name}"
            meta_response = await client.get(meta_url, headers=headers)
            
            if meta_response.status_code == 404:
                raise HTTPException(status_code=404, detail="Target repository not found on GitHub.")
            meta_response.raise_for_status()
            
            default_branch = meta_response.json().get("default_branch", "main")

            # STEP B: Fetch the complete directory tree using the recursive Trees API
